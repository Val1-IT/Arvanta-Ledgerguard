...
// does, and only via a Pool passed in by the caller (never a module-level
// global connection).
// ---------------------------------------------------------------------------

export async function fetchProducts(pool: Pool): Promise<ProductRecord[]> {
...
export async function fetchProductUnits(pool: Pool): Promise<ProductUnitRecord[]> {
  const { rows } = await pool.query(
